<?php
	...
	$sql = "select distinct on_stop_id, count(on_stop_id) as count from model_legs where run_id = $run_id and mode = 'BUS' group by on_stop_id order by count desc";
 	...
 	$sql = "select distinct route, count(route) as count from model_legs where run_id = 29 and mode = 'BUS' group by route order by count desc";
 	$rs=mysql_query($sql) or die($sql." ".mysql_error());
 	while($row = mysql_fetch_assoc($rs)){
 		$data['routes'][] = $row;
 	}
	
 	$sql = "select distinct gtfs_trip_id, route ,count(gtfs_trip_id) as count from model_legs where run_id = 27 and mode = 'BUS' group by gtfs_trip_id order by route,count desc";
 	$rs=mysql_query($sql) or die($sql." ".mysql_error());
 	//$data['sql'] = $sql;
 	while($row = mysql_fetch_assoc($rs)){
 		$data['trips'][] = $row;
